...
class FluidQuantity {
  ...
  void euler(double &x, double &y, double timestep, const FluidQuantity &u,
...
public:
  ...
  const double *src() const { return _src; }

  /* Read-only and read-write access to grid cells */
  double at(int x, int y) const { return _src[x + y * _w]; }

  double &at(int x, int y) { return _src[x + y * _w]; }
...
public:
  ...
  void addInflow(double x, double y, double w, double h, double d, double u,
                 double v) {
